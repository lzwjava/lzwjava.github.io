...
Download: Obtain the latest version of YourKit Java Profiler from the official website (https://www.yourkit.com/java/profiler/download/). A 15-day free trial is available.
Install:
...
Verify Installation: Ensure the profiler is installed correctly by running java -agentpath:<full agent library path> -version. This checks if the profiler agent loads correctly.
...
Locate the Agent Library:
The agent library is located in <YourKit Home>/bin/<platform>/libyjpagent.so (Linux) or libyjpagent.dll (Windows).


Configure the JVM:
Add the agent to the JVM startup command:java -agentpath:<full agent library path> YourMainClass


Example for Linux:java -agentpath:/home/user/yjp-2025.3/bin/linux-x86-64/libyjpagent.so YourMainClass
...
Install the YourKit plugin for your IDE (Eclipse, IntelliJ IDEA, or NetBeans) via the respective plugin marketplace.
Configure the plugin to point to the YourKit installation directory.
Use the IDE’s profiling option to start your application with YourKit attached.

Remote Profiling

Ensure SSH Access: You need SSH access to the remote server.
Copy the Agent:
Copy the appropriate agent library to the remote server.
Example for Docker:docker cp libyjpagent.so <container_id>:/path/to/agent
...
Use Triggers: Automatically start profiling based on high CPU usage or specific method calls.
...
Capture Snapshots:
Use the UI or command-line tool:java -jar <YourKit Home>/lib/yjp-controller-api-redist.jar localhost 10001 capture-performance-snapshot
