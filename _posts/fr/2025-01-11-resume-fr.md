...
* A lu plus de 320 livres couvrant les manuels, le développement personnel, les affaires, la technologie et l'histoire.
...
- A géré le cycle de développement complet, du développement local aux tests UAT et aux mises en production. A également résolu des problèmes tels que les deadlocks et les procédures IBM Db2. Copilot a été largement utilisé pour des tâches étendues telles que l'analyse des causes racines, la génération de scripts Python, et la documentation d'environ 50 guides techniques ; certains de ces guides sont devenus des références clés pour les collègues confrontés à des défis similaires.
- Dans l'environnement contraint de la banque, j'ai essayé beaucoup de nouvelles façons de faire les choses. Cela incluait l'utilisation intermittente de VSCode ou du plugin Copilot d'IntelliJ pour exploiter la puissance de l'assistant IA interne, l'introduction du YourKit Java Profiler pour aider à mesurer les performances du logiciel, l'utilisation de quatre outils de base de données ou la création de scripts pour effectuer certaines tâches de la meilleure manière, et l'accumulation de centaines de logs dans un répertoire au fur et à mesure que le voyage continuait, pour aider à l'analyse ultérieure.
...
* A analysé et réimplémenté certaines parties centrales autour de 30 projets d'exemple sur le machine learning, provenant de PyTorch, TensorFlow tutorials, cours en ligne Coursera ou autres projets open-source. A obtenu deux certificats de complétion de cours de Coursera sur la spécialisation Machine Learning et la spécialisation Deep Learning.
...
* Développement de site web LED Sign (lvchensign.com) : A développé un site web pour une entreprise de fabrication d'enseignes LED en utilisant Bootstrap, HTML et JavaScript. A implémenté des fonctionnalités pour présenter les produits.
...
* Web Scraper pour la collecte de données commerciales : A développé un web scraper utilisant Python et Selenium pour collecter des données commerciales pour une entreprise de tissu non tissé. A automatisé l'extraction de données et la navigation de page, traité et stocké les données dans une base de données SQLite, et généré des rapports pour l'analyse commerciale.
...
* A reçu le troisième prix dans le concours de créativité des œuvres numériques "Jeunesse, Santé, Nouveaux Médias" organisé par l'UNESCO et l'Université de la Communication de Chine pour l'œuvre numérique "Maman Timide de Dire".
...
* Configuration de Diffusion en Direct avec OBS, SRS, et FFmpeg
