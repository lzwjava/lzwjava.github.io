...
lang: fr
layout: post
title: Introduction au *Machine Learning* avec PyTorch
translated: true
---
...
Après avoir recherché ce message d'erreur sur Google, il s'avère que notre fichier s'appelait également `torch`, ce qui a provoqué un conflit de nom. Après l'avoir renommé, cela fonctionne correctement.
...
# device = torch.device("cuda:0") # Décommentez ceci pour exécuter sur GPU
...
learning_rate = 1e-6
for t in range(2000):
    # Passage avant : calculer y prédit
...
Remarquez que ce sont deux façons différentes d'effectuer des calculs.

Dans ces exemples, un ensemble de valeurs x et y est d'abord généré. Ensuite, on suppose qu'il s'agit d'une équation cubique. Ensuite, certaines méthodes sont utilisées pour calculer de manière itérative les coefficients. Quels sont ces algorithmes ? Remarquez qu'il boucle `2000` fois, devenant plus précis à chaque itération. Nous n'approfondirons pas les détails ici.

## Enfin

Actuellement, nous ne comprenons pas comment les calculs de l'apprentissage automatique fonctionnent en coulisses. Cependant, ce n'est pas important pour l'instant. Avec les connaissances ci-dessus, nous pouvons déjà faire beaucoup de choses. L'apprentissage automatique peut également être utilisé pour traiter du texte, de l'audio, et plus encore. Après avoir exploré des dizaines d'exemples, il ne sera pas trop tard pour apprendre la théorie.

## Exercices

* Les étudiants doivent explorer comme montré ci-dessus.
```