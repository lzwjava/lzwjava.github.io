...
* La différence entre utiliser `replace` ou `startWith` suivi de `slice` est que le premier ignore la position de la chaîne. Appliquez cette réflexion à chaque détail de la programmation.  

* Minimisez les valeurs possibles pour un élément ; utilisez une seule valeur par cas. N'utilisez pas `null` si vous avez déjà `false`. Si vous avez un drapeau traduit `true` ou `false`, assurez-vous de ne pas traiter l'absence de drapeau traduit comme `false`.  
