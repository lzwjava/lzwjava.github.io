...
* Utilisez Gemini ou Grok pour générer des images de célébration de festival avec des invites comme "Générez une image de Nouvel An du Serpent Lunaire joyeuse avec les noms du texte inclus".
...
* Lors de l'utilisation de l'API Deepseek ou Mistral pour traduire des titres avec des invites comme `Vous êtes un traducteur professionnel. Vous traduisez un fichier markdown pour un article de blog Jekyll de l'anglais vers le chinois. {text}`, cela peut conduire à des traductions incorrectes. En plus du texte que vous fournissez, la sortie inclut souvent une traduction excessive.
...
* Lors de la fourniture d'un contexte ample pour des tâches comme la traduction ou la génération de paroles de dialogue, évitez d'utiliser les fonctionnalités de chaîne de pensée, car cela peut être lent et conduire à des réponses verbeuses ou peu utiles.
...
* Utilisez les chatbots IA à tour de rôle pendant une semaine afin de comprendre leurs différences. Ne vous en tenez pas à un seul outil IA ou au meilleur outil si vous voulez en apprendre davantage.
...
* Au lieu d'utiliser une pensée approfondie ou une Chaîne de Pensée (CoT) pour résoudre des problèmes difficiles, vous pouvez parfois simplifier les problèmes pour laisser les modèles standard les résoudre.
...
* Lorsque vous travaillez avec les dernières informations—comme les bibliothèques, le code ou la documentation nouvellement sortis—il est préférable de vérifier avec plusieurs chatbots IA, car leurs coupures de connaissances et leurs réponses peuvent différer de manière subtile.

* Après avoir écrit un essai, nous pouvons utiliser les chatbots IA pour corriger la grammaire et Git pour visualiser les différences.
