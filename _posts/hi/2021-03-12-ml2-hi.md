...
```markdown
यह पोस्ट मूल रूप से चाइनीज़ भाषा में लिखी गई थी। इसे अन्य भाषाओं में अनुवाद की सुविधा के लिए अंग्रेज़ी में अनुवादित किया गया है।

---

## PyTorch

आइए इसे इंस्टॉल करें। यह Python संस्करण `3.9` को सपोर्ट करता है।
...
इस त्रुटि संदेश को गूगल करने के बाद पता चला कि हमारी फ़ाइल का नाम भी `torch` रखा हुआ था, जिससे नाम का टकराव हो रहा था। नाम बदलने के बाद यह सही तरीके से काम करता है।
...
# यादृच्छिक रूप से वेट्स इनीशियलाइज़ करें
...
learning_rate = 1e-6
for t in range(2000):
    # फॉरवर्ड पास: अनुमानित y की गणना करें
    y_pred = a + b * x + c * x ** 2 + d * x ** 3

    # लॉस की गणना करें और प्रिंट करें
    loss = (y_pred - y).pow(2).sum().item()
    if t % 100 == 99:
        print(t, loss)

    # बैकप्रोपेगेशन: लॉस के संबंध में a, b, c, d के ग्रेडिएंट की गणना करें
    ...
    # ग्रेडिएंट डिसेंट का उपयोग करके वेट्स अपडेट करें
...
आइए केवल `numpy` लाइब्रेरी का उपयोग करके कोड देखें।
...
ध्यान दें कि ये दो अलग-अलग तरीके हैं जिनसे गणनाएँ की जा सकती हैं।

इन उदाहरणों में, पहले x और y के मानों का एक सेट बनाया जाता है। फिर, इसे एक घन समीकरण माना जाता है। इसके बाद, कुछ तरीकों का उपयोग करके पुनरावृत्त रूप से गुणांक की गणना की जाती है। ये एल्गोरिदम क्या हैं? ध्यान दें कि यह `2000` बार लूप करता है, हर पुनरावृत्ति के साथ अधिक सटीक होता जाता है। यहाँ हम इसके विवरण में नहीं जाएंगे।

## अंत में

वर्तमान में, हम नहीं समझते कि मशीन लर्निंग की गणनाएँ पर्दे के पीछे कैसे काम करती हैं। हालांकि, यह अभी महत्वपूर्ण नहीं है। ऊपर दिए गए ज्ञान से, हम पहले से ही कई काम कर सकते हैं। मशीन लर्निंग का उपयोग टेक्स्ट, ऑडियो और अन्य चीज़ों को प्रोसेस करने के लिए भी किया जा सकता है। दर्जनों उदाहरणों का अन्वेषण करने के बाद, सिद्धांत सीखने में देर नहीं होगी।

## अभ्यास

* छात्रों को ऊपर दिखाए अनुसार अन्वेषण करना चाहिए।
```