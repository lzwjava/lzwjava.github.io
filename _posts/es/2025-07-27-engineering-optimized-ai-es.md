...
He trabajado como contratista en varios proyectos bancarios en los últimos años. He observado una arquitectura bancaria impresionante, microservicios, un diseño efectivo multipaís para eliminar la duplicación en la medida de lo posible, un marco fundamental robusto construido sobre Spring, y una fuerte gobernanza con configuración centralizada.

Aunque su arquitectura bancaria es impresionante, si tuviéramos que empezar hoy, consideraría optimizar para LLM/IA. Esto implicaría una mejor ingeniería de contexto, una mejor separación de contextos y priorizar el pensamiento centrado en IA sobre el diseño centrado en humanos. Aunque Spring ofrece múltiples niveles y buena abstracción, puede ser difícil para LLM/IA navegar.

Creo que deberíamos apuntar a estructuras más planas, similares a una organización plana. Esto significa usar solo dos niveles: el primer nivel llama al segundo nivel. En una función, es mejor llamar a otras 50 funciones directamente en lugar de tener 50 niveles o pilas anidadas. La IA/LLM lucha por juzgar o inferir estructuras complejas y anidadas, pero se desempeñan bien manejando funciones más pequeñas de 100 a 200 líneas de código. Python es adecuado para llamar e importar de otros archivos.

Una razón por la que el código de Python es más fácil que el de Java es que su gestión de dependencias es simple. Solo necesitas usar `pip install` para agregar una dependencia. Con Maven, debes escribir la dependencia en un archivo POM XML y luego usar `mvn compile` para que Maven descargue las dependencias.
...
¿Podríamos, desde el lado del usuario, organizar el código específicamente para reducir el uso de tokens? Para este punto, el enfoque de tener 1000 archivos de Python con cada uno 100 líneas de código es bueno para este propósito. Porque puedes verificar funciones y archivos de código fácilmente antes de dejar que otros códigos de Python los llamen.

Pero un problema es que si quieres cambiar varios archivos de código juntos, no es fácil hacerlo. Para una manera simple, puedes copiar el código a los chatbots de IA y dejar que ellos te digan cómo editar el código en estos archivos.
...
Y para archivos HTML, deberíamos hacer alguna separación. En lugar de escribir cientos de archivos de JavaScript junto con cientos de archivos HTML, haciendo que sea fácil superar las 1000 líneas de código, deberíamos usar `import` para JavaScript para gestionar esto. Para el código de JavaScript, podemos usar los métodos anteriores para separarlo.

Queremos estructurar el código de manera que la IA nos ayude fácilmente a agregar, editar, eliminar y ejecutar código. Este es el comienzo. Imagina un día en que todo el código pueda ser generado o corregido por la IA fácilmente. El mundo estará altamente digitalizado.

Imagínate a mí mismo escribiendo 100 grandes proyectos de software y proporcionando APIs para conectarlos con otros. Esto incluye mi agenda diaria; yo mismo soy como una empresa tecnológica de 1000 empleados en la actualidad. Están personalizados para mis necesidades, para ganar dinero o gastar dinero para mi beneficio. Esto es realmente asombroso.