...
Vamos a instalarlo. Esto soporta la versión de Python `3.9`.
...
Después de buscar este mensaje de error en Google, resultó que nuestro archivo también se llamaba `torch`, lo que causó un conflicto de nombres. Después de renombrarlo, funciona correctamente.
...
learning_rate = 1e-6
for t in range(2000):
    # Paso hacia adelante: calcular y predicha
    y_pred = a + b * x + c * x ** 2 + d * x ** 3

    # Calcular e imprimir pérdida
    loss = (y_pred - y).pow(2).sum().item()
    if t % 100 == 99:
        print(t, loss)

    # Retropropagación para calcular gradientes de a, b, c, d con respecto a la pérdida
...
Veamos un código que usa solo la librería `numpy`.
...
Observa que estos son dos métodos diferentes para realizar cálculos.

En estos ejemplos, primero se genera un conjunto de valores x e y. Luego, se asume que es una ecuación cúbica. A continuación, se utilizan algunos métodos para calcular iterativamente los coeficientes. ¿Qué algoritmos son estos? Observa que el bucle se ejecuta `2000` veces, volviéndose más preciso con cada iteración. No profundizaremos en los detalles aquí.

## Finalmente

Actualmente, no entendemos cómo funcionan los cálculos del aprendizaje automático detrás de escena. Sin embargo, eso no es importante por ahora. Con los conocimientos anteriores, ya podemos hacer muchas cosas. El aprendizaje automático también puede usarse para procesar texto, audio y más. Después de explorar decenas de ejemplos, no será demasiado tarde para aprender la teoría.

## Ejercicios

* Los estudiantes deben explorar como se muestra anteriormente.