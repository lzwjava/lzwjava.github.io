...
Sin embargo, para Facebook, Google o Apple Pay, es algo simple. Alguien podría decir que no son aplicaciones financieras; tienen algunas reglas de cumplimiento que deben seguir. No es cierto. El cumplimiento a menudo significa el servidor de base de datos, o la base de datos, o algunos datos que los departamentos gubernamentales quieren verificar o para que las empresas de auditoría realicen auditorías. Sin embargo, otros esfuerzos son los mismos. El software es muy flexible. Deberíamos permitir que el código esté en el mismo repositorio, deberíamos usar la configuración de fuentes de datos para alojar datos de diferentes regiones y deberíamos compartir el mismo código, el mismo diseño, el mismo flujo de trabajo y las mismas pruebas en la mayor medida posible.

Apple Pay es un buen ejemplo de esto. La App Store también es un buen ejemplo de esto. Ellos también sirven a cada país.

Probablemente hay algunos proyectos en grandes empresas tecnológicas que usan continentes para separar, como Asia y el área del Pacífico, América del Norte. Para estos también.
...
Para proyectos multi-regional, en proyectos de Spring Boot, podemos usar sus alias y diferentes inicializaciones de objetos para hacerlo. Podemos usar propiedades o configuración YAML de manera inteligente. Podemos poner toda la lógica diferente basada en la región en algunos módulos o clases específicos.
...
No solo para la codificación, el despliegue y el mantenimiento de lanzamientos, sino también para la extensibilidad. Considera cómo agregar un nuevo país o región. ¿Cuánto esfuerzo requerirá eso? Si es mínimo o solo implica alguna configuración, entonces nuestro diseño es excelente. Si toma unos pocos meses, también es aceptable. Si toma varios años, ¿todavía procederemos con ello?

En el ensayo de Yin Wang, [Sobre Linux, Windows y Mac](https://www.yinwang.org/blog-cn/2013/03/07/linux-windows-mac), mencionó que un diseñador de Adobe le dijo que pasaron dos años migrando Photoshop de Windows a macOS.
