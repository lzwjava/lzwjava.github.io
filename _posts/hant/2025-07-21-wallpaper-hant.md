---
audio: false
generated: false
image: true
lang: hant
layout: post
title: 牆紙變換快捷方式 (iOS)
translated: true
---

[https://www.icloud.com/shortcuts/f972a5bc6f9c43109f7461465b4be2ee](https://www.icloud.com/shortcuts/f972a5bc6f9c43109f7461465b4be2ee)

這個快捷指令會更改 iOS 的桌面背景。我已經使用了幾個月，效果非常好。在每週特定的日子，下午 3:14，這個快捷指令會自動在我的 iPhone 上執行。它簡單又有效。它會從 Unsplash 下載一張圖片並設定為桌面背景。請注意，你需要先安裝 Unsplash 應用程式。

使用類似的方法通常比購買或購買桌面背景應用程式來做這件事更好。

我是從 Andrej Karpathy 那裡學到設定下午 3:14 的提醒的，他用這個時間來拍攝照片。

{: .centered }
![](assets/images/wallpaper/w0.jpg){: .responsive }
*來源：自拍*{: .caption }

{: .centered }
![](assets/images/wallpaper/w1.jpg){: .responsive }
*來源：自拍*{: .caption }

{: .centered }
![](assets/images/wallpaper/w2.jpg){: .responsive }
*來源：自拍*{: .caption }