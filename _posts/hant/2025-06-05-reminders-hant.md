...
lang: hant
layout: post
title: 透過Telegram簡化提醒功能
translated: true
---

在這個項目中，我利用GitHub Actions和Telegram機器人建立了一個自動提醒系統，以確保我的日常和月度任務按時完成。通過設定cron排程，我配置了工作相關任務的提醒，例如在企業微信打卡、提交工時表和查閱薪資，以及個人事務如探訪家人、在京東購物，甚至與伴侶一起看電視。系統使用Python腳本通過Telegram的Bot API發送訊息，並將環境變數安全地儲存在GitHub Secrets中。這樣的設定確保我不會錯過任何重要期限或個人承諾，將科技與日常生活結合以達到最高效率。
...
on:
  schedule:
    ...
    # 每天上午11點（北京時間）執行。
    - cron: '0 3 * * *'
    # 提醒隔天去父母家：每週二、三、四晚上9點（北京時間）。
    - cron: '0 13 * * 2-4'
    # 提醒隔天回自己家：每週日、一、五、六晚上9點（北京時間）。
    - cron: '0 13 * * 0,1,5,6'
    # 提醒在京東購買生鮮直供：每週三晚上9點（北京時間）。
    - cron: '0 13 * * 3'
    # 提醒在京東購買速食：每週五晚上9點（北京時間）。
    - cron: '0 13 * * 5'
    # 提醒副學士學位考試報名：每年3、4、9、10月的每週一中午1點（北京時間）。
    - cron: '0 5 * 3,4,9,10 1'
    # 提醒提交Clarity工時表：每週五下午5點（台北時間）。
    - cron: '0 9 * * 5'
    # 提醒提交供應商工時表：每月25日凌晨12點（台北時間）。
    - cron: '0 16 25 * *'
...
jobs:
  send-reminders:
    ...
    steps:
      ...
      - name: 執行Telegram腳本發送Clarity工時表提交提醒
        run: python scripts/release/reminders_bot.py --job send_message --message "提交Clarity工時表"
...
def send_reminder(message):
    ...
    if TELEGRAM_BOT2_API_KEY and TELEGRAM_CHAT_ID:
        ...
        print("TELEGRAM_BOT2_API_KEY和TELEGRAM_CHAT_ID未設定。")
