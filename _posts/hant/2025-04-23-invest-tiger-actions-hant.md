...
lang: hant
layout: post
title: 每週股票投資與 TigerOpen API 及 GitHub Actions
translated: true
---

我創建了一個 Python 腳本和一個 GitHub Actions 工作流程來自動化我的每週投資計劃，每週三晚上 10:35 UTC 買入一股 NVIDIA 股票。我選擇週三是因為在 2025 年，該日沒有假期，確保一致執行。

## 概述

該腳本使用 TigerOpen API 下單 NVIDIA 股票並監控其狀態。GitHub Actions 工作流程按計劃運行腳本，安全地處理設置和驗證。以下是兩個組件的詳細資料。

## Python 腳本

該腳本下單買入一股 NVIDIA 股票，最多檢查 60 秒的狀態，如果未成交則取消。
...
該工作流程每週三 14:35 UTC（10:35 PM UTC）運行一次，設置環境，安裝依賴項並執行腳本。
