...
這些軟體代表了我多年來有幸與各大知名公司和夥伴合作的多元化專案。
...
[SegmentFault：2014年AVOS雲端黑客馬拉松成果回顧](https://segmentfault.com/a/1190000000427302)
...
LeanChat是一個包含好友管理、群組管理、基於位置的消息、附近的人、個人資料和登錄/註冊功能的聊天應用程式，充分利用了LeanCloud的儲存和通訊功能。
...
- 確保LeanCloud服務如物件儲存、文件儲存和消息等的無縫整合。
...
開發了一個Chrome外掛程式，當瀏覽器因太多標籤而變得雜亂時，自動關閉最舊的標籤，從而提高瀏覽器性能和用戶體驗。
...
創建了一個Xcode外掛程式，專為在當前存儲庫中無縫導航到關鍵GitHub功能而設計。該外掛程式允許用戶快速訪問GitHub歷史記錄、責備、拉取請求、問題和通知。
...
開發了江蘇電視台超級大腦節目的微信小程序，為用戶提供互動式謎題。主持人通過口播廣播引導用戶，提出特定技術挑戰。
...
ShowMeBug是一個技術能力評估平台，支持實際編程，實現高效且準確的技術人才評估和招聘。
...
陸晨招牌專注於為各種企業和活動生產廣告招牌。以品質和創意為核心，致力於提供提升品牌形象和吸引觀眾的視覺解決方案。

開發了陸晨招牌的前端，一個專門從事廣告招牌生產的網站。
...
在[GitHub](https://github.com/lzwjava/lvchensign)上開源，40次提交。在線頁面於[lvchensign.com](http://lvchensign.com)。
...
為DBS Client Connect貢獻，這是一個基於AI和數據驅動的客戶關係管理平台，專注於股票交易微服務。
...
DBS DigiBank CN重新思考並重新設計，讓您享受更快速和方便的銀行服務。

為DBS DigiBank CN貢獻，專注於共同基金微服務的後端開發，作為協作團隊的一部分。
...
- 開發一個工具來自動生成測試工具Karate的測試，從而簡化了測試流程並提高了測試覆蓋率。
...
**職責：**
- 與後端工程師團隊合作開發自動補款功能，當用戶的餘額低於一定金額時，自動從用戶的信用卡或借記卡補款。
- 協作監控來自Azure EventHub的支付後事件，確保無縫整合，並在用戶設置自動補款配置時檢查補款。
...
- 使用Prometheus進行監控，使用ELK堆棧進行日誌管理。
- 實現端到端追蹤ID解決方案以跟蹤系統中的請求。

## 金融轉型平台 - HSBC，後端，2025.02 - 至今
...
**職責：**
- 作為TEKsystems外包的後端工程師，開發和優化HSBC企業技術部門的金融轉型平台的後端系統。
- 維護財務數據處理功能，包括導入、驗證和導出，同時增強新財務標頭的提交和批准工作流程，利用會計、總賬和銀行系統的專業知識。
- 管理從本地開發到UAT測試和生產發布的整個開發生命週期，利用Copilot進行根本原因分析、生成Python腳本和記錄約50份技術指南，這些指南成為同事的關鍵參考資源。
- 使用AI工具如Copilot、性能分析工具YourKit和系統測試開發創新的銀行技術解決方案，同時作為自主AI代理，分析項目架構、與團隊協作並通過全面的日誌記錄和文檔改進。
- 探索在主要金融項目中實施AI軟體代理，使用AspectJ收集大量日誌並將調試狀態轉換為文本，為AI聊天機器人提供上下文。