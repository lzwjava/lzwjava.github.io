...
PDFファイルは[こちら](/assets/resume/Zhiwei.Li.Portfolio.pdf)から入手できます。

このソフトウェアコレクションは、これまでに様々な名門企業やパートナーと共に携わってきた多様なプロジェクトを表しています。
...
解決したアルゴリズム問題の範囲は、データ構造、グラフ、数学、文字列処理、幾何学など、さまざまな分野を含み、いくつかの高度または専門的なトピックも含まれています。

## 花の認識, Android, 2014.4 - 2014.8

写真を撮影し、認識を助けるために円を描くことでユーザーが花を識別できるように設計された花の認識Androidアプリケーションを開発しました。
...
[GitHubでオープンソース](https://github.com/lzwjava/flower-recognition), 33コミット。

## iword, Android, 2014.3

映画のクリップを観ながら英語を学べるAndroidアプリケーション。
...
**成果:** 2014年AVOSクラウドハッカソンで第三賞を受賞。

[SegmentFault: 2014年AVOSクラウドハッカソンの成果レビュー](https://segmentfault.com/a/1190000000427302)

## Bjfu Assistant, Android, 2014.1 - 2014.6

成績の確認や同窓会フォーラムへの参加など、キャンパスアシスタント機能を統合したAndroidアプリケーション。
...
LeanChatは、友達管理、グループ管理、位置情報ベースのメッセージ、近くの人、個人プロフィール、ログイン/登録機能などの機能を備えたチャットアプリケーションで、LeanCloudのストレージと通信機能をフル活用しています。

**使用技術:** Android SDK, Java, LeanCloud

**責任:**
- 友達管理、グループ管理、位置情報ベースのメッセージ機能を開発しました。
- 個人プロフィールとログイン/登録機能を実装しました。
- LeanCloudのストレージと通信機能を活用しました。

[GitHubでオープンソース](https://github.com/lzwjava/leanchat-android), 412コミット。
...
LeanCloudのバックエンドサービスを利用したメッセージングとストレージのためのLeanChatのiOSバージョンを開発しました。

**使用技術:** iOS SDK, Objective-C, LeanCloud

**責任:**
- 友達とグループ管理機能を構築しました。
- 位置情報ベースのメッセージと近くの人機能を作成しました。
- 個人プロフィールとログイン/登録画面を設計しました。

[GitHubでオープンソース](https://github.com/lzwjava/leanchat-ios), 446コミット。
...
ブラウザが多くのタブで混雑したときに自動的に最も古いタブを閉じ、ブラウザのパフォーマンスとユーザーエクスペリエンスを向上させるChromeプラグインを開発しました。

**使用技術:** JavaScript, Chrome API

[GitHubでオープンソース](https://github.com/lzwjava/TabsKiller), 19コミット。
...
現在のリポジトリ内でGitHubの主要機能にシームレスにナビゲートするために設計されたXcodeプラグインを作成しました。このプラグインは、ユーザーがGitHubの履歴、ブレーム、プルリクエスト、イシュー、通知に素早くアクセスできるようにします。

**使用技術:** Objective-C, XcodeプラグインAPI

[GitHubでオープンソース](https://github.com/lzwjava/Reveal-In-GitHub), 57コミット。
...
- スケーラブルで信頼性の高いインフラを提供するためにAlibaba Cloudと統合しました。
...
* [code-review-web](https://github.com/lzwjava/code-review-web), 488コミット（そのうち302コミットは私によるもの）
...
Fun Liveは、ユーザーがプログラミングやデザインなどのさまざまな知識講座に参加できる知識ライブ放送プラットフォームです。ユーザーはライブセッションに参加するために料金を支払ったり、講師に報酬を与えたりできます。

**使用技術:** PHP, MySQL, Vue.js, CodeIgniter, Alibaba Cloud, SRSストリーミング, WeChat SDK

**責任:**
- バックエンドとフロントエンドの大部分のコードを開発しました。
- ライブストリーミングのためにOBSツールを統合しました。
- ユーザー登録、決済、通知機能を実装しました。
- 約30,000人のユーザーと数百万のページビューを達成し、大規模なユーザーエンゲージメントを達成しました。

GitHubでオープンソース。

* [live-server](https://github.com/lzwjava/live-server), 661コミット（そのうち555コミットは私によるもの）
* [live-mobile-web](https://github.com/lzwjava/live-mobile-web), 528コミット（そのうち426コミットは私によるもの）
...
Mianbao Liveは、コンテンツモネタイゼーションとソーシャルエコノミーのワンストップソリューションプロバイダーです。
...
江蘇テレビの超級大脳プログラムのためのWeChatミニプログラムを開発しました。このプログラムには、ユーザーにとって魅力的なパズルが含まれています。ホストは、ユーザーを口頭放送を通じてナビゲートし、特定の技術的な課題を提示します。
...
- Redisを使用して高い並列性の下で高いパフォーマンスと信頼性を確保しました。
...
- ライブストリーミングをゲームイベントと同期させるための技術ソリューションに貢献し、SEI（補助強化情報）を使用しました。
...
- ブラウザ内でDockerを使用してプログラミング環境を構築する技術研究を実施しました。
...
LvchenSignのフロントエンドを開発しました。広告看板の製造に特化したウェブサイト。

**使用技術:** HTML, JavaScript, CSS, Bootstrap

**責任:**
- ウェブサイトのフロントエンドを設計・実装しました。
- 製品を紹介する直感的でユーザーフレンドリーなインターフェースを作成しました。

[GitHubでオープンソース](https://github.com/lzwjava/lvchensign), 40コミット。オンラインページは[lvchesign.com](http://lvchensign.com)。
...
- Avaloq APIを統合し、基盤インフラを強化し、ユーザーエクスペリエンスを向上させました。

<div align="center">

<img src="/assets/images/portfolio/dbsconnect.webp"/><img/>
<p><a href="https://www.eddytandesign.com/client-serving-dbs">画像ソース: eddytandesign</a></p>
...
**責任:**
- 投資信託の購入と償還機能を強化し、ユーザーエクスペリエンスとシステムの信頼性を向上させました。
- 構造化投資商品、投資ポートフォリオ、取引リストに関連するマイクロサービスのバグ修正とコード強化に貢献し、全体的なシステムの安定性と正確性を向上させました。
- QPSのパフォーマンステストを実施し、ログを分析してレポートを作成し、チームメンバーと協力してパフォーマンスを最適化しました。
- Karateテストツールのためのテスト生成を自動化するツールを開発し、テストプロセスをストリームライン化し、テストカバレッジを向上させました。
...
<p><a href="https://www.dbs.com.cn/treasures/digital-service/digibank.html">画像ソース: DBS Digibank CN</a></p></div>

## PayMe by HSBC - 自動チャージ, バックエンド, 2022.12 - 2023.7

HSBCのPayMeは、友人への送金やオンライン/オフラインのモバイル決済を便利に行う方法です。

**使用技術:** Java, Spring, Kafka, Azure, Azure DevOps, AWS

**責任:**
- 自動チャージ機能を開発するためのバックエンドエンジニアチームと協力しました。この機能は、残高が一定額を下回ったときに、ユーザーのクレジットカードまたはデビットカードから自動的に資金をチャージします。
- Azure EventHubからの決済後イベントを監視し、ユーザーが自動チャージ設定をセットアップしたときに常にトップアップを確認できるように、シームレスな統合を確保しました。
- オブジェクト指向プログラミングの原則を適用して、自動チャージテーブルの変更に対する監査ログをエレガントに処理し、アスペクト指向プログラミングを使用して監査ログを監査しました。
- AWS移行プロセス中にチームを支援し、IAMとRDSの構成をトラブルシューティングし、スムーズな移行を確保しました。
...
<p><a href="https://payme.hsbc.com.hk/">画像ソース: PayMe by HSBC</a></p>
...
- プロンプト設定と管理のためにReactを使用してフロントエンドを構築しました。
- AWSにデプロイし、Nginxを使用して安全な通信を確保しました。
- Prometheusを使用してモニタリングを実装し、ELKスタックを使用してログ管理を実施しました。
- システム全体をトラッキングするためのエンドツーエンドのトレースIDソリューションを実装しました。

## 金融変革プラットフォーム - HSBC, バックエンド, 2025.02 - 現在
...
**責任:**
- HSBCのエンタープライズテクノロジーデパートメントのバックエンドエンジニアとして、金融変革プラットフォームのバックエンドシステムを開発および最適化しました。TEKsystemsを通じてアウトソーシングされました。
- 会計、総勘定元帳、銀行システムの専門知識を活かして、財務データ処理機能（インポート、検証、エクスポート）を維持し、新しい財務ヘッダーの提出と承認ワークフローを強化しました。
- ローカル開発からUATテスト、プロダクションリリースまでのフル開発ライフサイクルを管理し、Copilotを使用してルート原因分析、Pythonスクリプトの生成、約50の技術ガイドの文書化を行い、チームメイトの主要な参考資料となりました。
- CopilotのようなAIツールを使用して革新的な銀行技術ソリューションを開発し、YourKitを使用してパフォーマンスプロファイリングを行い、システマティックなテストを実施し、自律的なAIエージェントとして機能し、プロジェクトアーキテクチャを分析し、チームと協力し、包括的なロギングとドキュメンテーションを通じて改善しました。
- アスペクトJを使用して大量のログを収集し、デバッグ状態をテキストに変換し、AIチャットボットにコンテキストを提供することで、主要な金融プロジェクトにAIソフトウェアエージェントを実装する可能性を探求しました。