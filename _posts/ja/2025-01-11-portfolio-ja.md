...
このソフトウェアコレクションは、これまでに様々な尊敬される企業やパートナーと協力して取り組んできた多様なプロジェクトを表しています。

これらの協力関係から得られた機会と経験に感謝しており、今後の取り組みにおいてもスキルと専門知識を活かして貢献していきたいと考えています。
...
写真を撮影して認識を支援する円を描くことで、ユーザーが花を識別するのを助けるためのAndroidアプリケーションを開発しました。
...
映画のクリップを視聴することで英語を学習するのを助けるAndroidアプリケーション。
...
LeanChatは、友達管理、グループ管理、位置情報ベースのメッセージング、近くの人、個人プロフィール、ログイン/登録機能などの機能を備えたチャットアプリケーションで、すべてLeanCloudのストレージと通信機能を活用しています。
...
LeanCloudのバックエンドサービスを利用したメッセージングとストレージのためのLeanChatのiOS版を開発しました。
...
LeanCloudのJava SDKとObjective-C SDKの開発に参加し、開発者がLeanCloudサービスをアプリケーションに簡単に統合できるツールを提供しました。
...
ブラウザが多くのタブで混雑している場合に自動的に最も古いタブを閉じ、ブラウザのパフォーマンスとユーザーエクスペリエンスを向上させるChromeプラグインを開発しました。

**使用技術:** JavaScript, Chrome API

GitHubでオープンソース[GitHub](https://github.com/lzwjava/TabsKiller), 19コミット。
...
GitHubでオープンソース[GitHub](https://github.com/lzwjava/Reveal-In-GitHub), 57コミット。
...
CodeReviewは、コードレビュー、コミュニケーション、共有のためのプロフェッショナルプラットフォームです。エンジニアは、コードの品質を向上させるために、専門家によるコードレビューを提出できます。
...
Fun Liveは、プログラミングやデザインなどのさまざまな知識講義に参加できる知識ライブ放送プラットフォームです。ユーザーはライブセッションに参加するために料金を支払ったり、講師に報酬を与えたりできます。

**使用技術:** PHP, MySQL, Vue.js, CodeIgniter, Alibaba Cloud, SRSストリーミング, WeChat SDK

**責任:**
- バックエンドとフロントエンドのほとんどのコードを開発しました。
- ライブストリーミングにOBSツールを統合しました。
- ユーザー登録、決済、通知機能を実装しました。
- 約30,000人のユーザーと数百万のページビューを達成し、大きなユーザーエンゲージメントを実現しました。
...
江蘇テレビの超級大脳番組のためのWeChatミニプログラムを開発しました。ユーザーにとって魅力的なパズルを提供し、ホストがユーザーを口頭放送でナビゲートし、特定の技術的な課題を提示します。
...
- SEI（サプリメント強化情報）を使用してライブストリーミングとゲームイベントを同期するための技術ソリューションに貢献しました。
...
LvchenSignは、さまざまなビジネスやイベント向けの広告看板の製造に特化しています。品質と創造性に焦点を当て、ブランドを引き上げ、観客を魅了するインパクトのあるビジュアルソリューションを提供することを目指しています。

広告看板の製造に特化したウェブサイトのフロントエンドを開発しました。
...
GitHubでオープンソース[GitHub](https://github.com/lzwjava/lvchensign), 40コミット。オンラインページは[lvchesign.com](http://lvchensign.com)。
...
DBS DigiBank CNは、より速く便利な方法でモバイルバンキングを楽しむために再考され、再設計されました。

DBS DigiBank CNに貢献し、投資信託マイクロサービスのバックエンド開発に焦点を当て、協力チームの一員として取り組みました。
...
- パフォーマンステストを実施し、ログを分析してQPSレポートを生成し、チームメンバーと協力してパフォーマンスを最適化しました。
...
PayMeは、HSBCが提供する友達への送金や、オンラインおよびオフラインの両方のモバイル決済を行う便利な方法です。
...
- Azure EventHubからの決済後イベントを監視し、ユーザーが自動チャージ設定をセットアップした際にチャージを確認するためにシームレスに統合しました。
- オブジェクト指向プログラミングの原則を適用して、自動チャージテーブルの変更に対する監査ログを生成するためにアスペクト指向プログラミングを利用しました。
- AWS移行プロセス中にチームを支援し、IAMとRDSの設定をトラブルシューティングして、スムーズな移行を確保しました。
...
ClaudeのAPIを使用したパーソナライズされたストーリーのためのAIパワードストーリーボットの開発に貢献しました。
...
- システム全体のリクエストを追跡するためのエンドツーエンドのトレースIDソリューションを実装しました。
...
- 会計、総勘定元帳、銀行システムの専門知識を活かし、金融データ処理機能（インポート、検証、エクスポート）を維持し、新しい金融ヘッダーの提出と承認ワークフローを強化しました。
- ローカル開発からUATテスト、プロダクションリリースまでのフル開発ライフサイクルを管理し、Copilotを使用してルート原因分析、Pythonスクリプトの生成、約50の技術ガイドの文書化を行い、チームメイトの主要な参考資料となりました。
- CopilotのようなAIツールを使用して革新的な銀行技術ソリューションを開発し、YourKitを使用してパフォーマンスプロファイリングを行い、システマティックなテストを行いました。また、プロジェクトアーキテクチャを分析し、チームと協力し、包括的なロギングと文書化を通じて改善する自律的なAIエージェントとして機能しました。
