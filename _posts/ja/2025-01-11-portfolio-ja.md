...
写真を撮影し、認識を助けるために円を描くことで、ユーザーが花を識別するのを支援する花の認識Androidアプリケーションを開発しました。
...
LeanChatのiOSバージョンを開発しました。このチャットアプリケーションは、メッセージングとストレージのためにLeanCloudのバックエンドサービスを利用しています。
...
- 約30,000人のユーザーと数百万のページビューを実現し、大きなユーザーエンゲージメントを達成しました。
...
Mianbao Liveは、コンテンツモネタイゼーションとソーシャルエコノミーの一元化ソリューションプロバイダーです。
...
- ライブストリーミングをゲームイベントと同期させるための技術ソリューションにSEI（補助強化情報）を使用しました。
...
## ShowMeBug - Enterprise WeChat Entrance, フルスタック, 2021.7 - 2021.9
...
DBS DigiBank CNに貢献し、投資信託マイクロサービスのバックエンド開発を担当しました。

**使用技術:** Java, Spring Cloud, Jenkins, Pivotal Cloud Foundry, Kibana

**責任:**
- 投資信託の購入と償還機能を強化し、ユーザー体験とシステムの信頼性を向上させました。
- 構造化投資商品、投資ポートフォリオ、取引リストに関連するマイクロサービスのバグ修正とコード強化を行い、全体的なシステムの安定性と正確性に貢献しました。
...
PayMeは、HSBCが提供する友達への送金や、オンラインおよびオフラインの両方の店舗でモバイル決済を行う便利な方法です。

**使用技術:** Java, Spring, Kafka, Azure, Azure DevOps, AWS

**責任:**
- バックエンドエンジニアチームと協力してAuto Top Up機能を開発しました。この機能は、残高が一定額を下回った場合に、自動的にクレジットカードまたはデビットカードから資金をチャージします。
- Azure EventHubからの決済後イベントを監視し、ユーザーが自動チャージ設定をセットアップした際に常にチャージを確認するために協力しました。
- オブジェクト指向プログラミングの原則を適用して、自動チャージテーブルの変更を監査するためのログをエレガントに処理しました。
- AWS移行プロセス中にチームを支援し、IAMとRDSの設定をトラブルシューティングし、スムーズな移行を確保しました。
...
- 会計、元帳、銀行システムの専門知識を活かし、金融データの処理機能（インポート、検証、エクスポート）を維持し、新しい金融ヘッダーの提出と承認ワークフローを強化しました。
- ローカル開発からUATテスト、プロダクションリリースまでのフル開発ライフサイクルを管理し、Copilotを使用してルート原因分析、Pythonスクリプトの生成、約50の技術ガイドの作成などのタスクを実行しました。これらのガイドはチームメイトにとって重要な参考資料となりました。
- CopilotのようなAIツールを使用して革新的な銀行技術ソリューションを開発し、YourKitを使用してパフォーマンスプロファイリングを行い、システマティックなテストを実施しました。また、プロジェクトアーキテクチャを分析し、チームと協力し、包括的なロギングとドキュメンテーションを通じて改善する自律型AIエージェントとして機能しました。
- アスペクトJを使用して広範なログを収集し、デバッグ状態をテキストに変換し、AIチャットボットにコンテキストを提供することで、主要な金融プロジェクトにAIソフトウェアエージェントを実装する可能性を探りました。