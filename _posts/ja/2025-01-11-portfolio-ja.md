...
映画のクリップを見ることで英語を学ぶのを助けるAndroidアプリケーション。
...
**成果:** 2014年AVOSクラウドハックソンで第三賞を受賞。

[SegmentFault: 2014年AVOSクラウドハックソンの成果レビュー](https://segmentfault.com/a/1190000000427302)
...
LeanChatは、友達管理、グループ管理、位置情報ベースのメッセージング、近くの人、個人プロフィール、ログイン/登録機能などの機能を備えたチャットアプリケーションで、すべてLeanCloudのストレージと通信機能をフル活用しています。
...
- スケーラブルで信頼性の高いインフラを実現するためにAlibaba Cloudと統合しました。
- ユーザー管理、コード提出とレビュープロセス、通知システム、支払い統合、イベント/ワークショップ管理を処理しました。
...
江蘇テレビの超級大脳プログラムのためのWeChatミニプログラムを開発しました。このプログラムには、ユーザーにとって魅力的なパズルが含まれています。ホストは、ユーザーを口頭放送でナビゲートし、特定の技術的な課題を提示します。
...
- Redisを使用して高い並列性下で高いパフォーマンスと信頼性を確保しました。
...
- ライブストリーミングとゲームイベントを同期させるための技術ソリューションに貢献し、SEI（補助強化情報）を使用しました。
...
**責任:**
- プロジェクトの交渉、チーム調整、クライアントとのコミュニケーションを管理しました。
...
- 構造化投資商品、投資ポートフォリオ、取引リストに関連するマイクロサービスのバグ修正とコード強化を行い、全体のシステムの安定性と正確性に貢献しました。
- QPSに関するレポートを生成するためにパフォーマンステストを実施し、ログを分析し、チームメンバーと協力してパフォーマンスを最適化しました。
- テストツールKarateのためのテスト生成を自動化するツールを開発し、テストプロセスをストリームライン化し、テストカバレッジを向上させました。
...
HSBCのPayMeは、友達への送金や、オンライン/オフラインの両方のメルチャントへのモバイル決済を行う便利な方法です。

**使用技術:** Java, Spring, Kafka, Azure, Azure DevOps, AWS

**責任:**
- 自動チャージ機能を開発するためにバックエンドエンジニアチームと協力しました。この機能は、残高が一定額を下回った場合に、ユーザーのクレジットカードまたはデビットカードから自動的に資金をチャージします。
- Azure EventHubからの支払い後のイベントを監視し、ユーザーが自動チャージ設定をセットアップした際にチャージが行われることを確保しました。
- オブジェクト指向プログラミングの原則を適用して、自動チャージテーブルの変更を監査するためのログをオーディットするためにアスペクト指向プログラミングを使用しました。
...
**責任:**
- HSBCのエンタープライズテクノロジーデパートメントのバックエンドエンジニアとして、TEKsystemsにアウトソーシングされて、金融変革プラットフォームのバックエンドシステムを開発および最適化しました。
- 会計、元帳、銀行システムの専門知識を活かし、金融データのインポート、検証、エクスポート機能を維持し、新しい金融ヘッダーの提出と承認ワークフローを強化しました。
- ローカル開発からUATテスト、プロダクションリリースまでのフル開発ライフサイクルを管理し、Copilotを使用してルート原因分析、Pythonスクリプトの生成、約50の技術ガイドの作成を行い、チームメイトにとって重要な参考資料となりました。
- CopilotのようなAIツールを使用して革新的な銀行技術ソリューションを開発し、YourKitを使用してパフォーマンスプロファイリングを行い、システマティックなテストを実施しました。プロジェクトアーキテクチャを分析し、チームと協力し、包括的なロギングとドキュメンテーションを通じて改善する自律型AIエージェントとして機能しました。
- 主要な金行プロジェクトでAspectJを使用して広範なログを収集し、デバッグ状態をテキストに変換し、AIチャットボットにコンテキストを提供するためのAIソフトウェアエージェントの実装を探求しました。