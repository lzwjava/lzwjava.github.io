...
これらの協力の機会と経験に感謝しており、今後もスキルと専門知識を活かして、今後の取り組みに貢献していきたいと考えています。
...
写真を撮影し、認識を助けるために円を描くことで、ユーザーが花を識別できるように設計された花の認識Androidアプリケーションを開発しました。
...
LeanChatのiOS版を開発しました。このチャットアプリケーションは、メッセージングとストレージのためにLeanCloudのバックエンドサービスを利用しています。
...
TabsKillerは、ブラウザに多くのタブが開かれていると自動的に最も古いタブを閉じ、ブラウザのパフォーマンスとユーザーエクスペリエンスを向上させるChromeプラグインを開発しました。
...
現在のリポジトリ内でGitHubの主要機能にシームレスにナビゲートできるように設計されたXcodeプラグインを開発しました。このプラグインは、ユーザーがGitHubの履歴、ブレーム、プルリクエスト、イシュー、通知に素早くアクセスできるようにします。
...
- Alibaba Cloudを統合してスケーラブルで信頼性の高いインフラストラクチャを実現しました。
...
* [code-review-web](https://github.com/lzwjava/code-review-web)、488コミット（302コミットは私によるもの）
...
* [live-server](https://github.com/lzwjava/live-server)、661コミット（555コミットは私によるもの）
* [live-mobile-web](https://github.com/lzwjava/live-mobile-web)、528コミット（426コミットは私によるもの）
...
Mianbao Liveは、ワンストップのコンテンツモネタイゼーションとソーシャルエコノミーソリューションプロバイダーです。
...
江蘇テレビの超級大脳プログラムのためのWeChatミニプログラムを開発しました。このプログラムには、ユーザーが楽しめる魅力的なパズルが含まれています。ホストは、ユーザーを口頭放送でナビゲートし、特定の技術的課題を提示します。
...
Chongdingは、HQ Triviaに似たモバイルクイズアプリで、ユーザーが賞金付きのライブクイズゲームに参加できます。
...
- ライブストリーミングとゲームイベントを同期させるための技術ソリューションに貢献し、SEI（サプリメント強化情報）を使用しました。
...
LvchenSignは、さまざまなビジネスやイベント向けの広告看板の生産に特化しています。品質と創造性に焦点を当て、ブランドを引き立て、観客を魅了するインパクトのあるビジュアルソリューションを提供することを目指しています。

LvchenSignのフロントエンドを開発しました。これは、広告看板の生産に特化したウェブサイトです。
...
AIとデータ駆動型の顧客関係管理プラットフォームであるDBS Client Connectに貢献し、株式取引マイクロサービスに焦点を当てました。

**使用技術:** Java、Spring Cloud、Jenkins、Pivotal Cloud Foundry

**責任:**
- 株式取引のバックエンドサービスを開発しました。株式表示、顧客表示、事前取引チェック、注文入力機能を作成しました。
- Avaloq APIを統合し、基盤インフラストラクチャを強化し、ユーザーエクスペリエンスを向上させました。
...
**責任:**
- 投資信託の購入と売却機能を強化し、ユーザーエクスペリエンスとシステムの信頼性を向上させました。
- 構造化投資商品、投資ポートフォリオ、取引リストに関連するマイクロサービスのバグ修正とコード強化に貢献し、全体的なシステムの安定性と正確性に寄与しました。
...
HSBCのPayMeは、友人に送金したり、オンラインとオフラインの両方でモバイル決済を行う便利な方法です。

**使用技術:** Java、Spring、Kafka、Azure、Azure DevOps、AWS

**責任:**
- バックエンドエンジニアチームと協力して、自動チャージ機能を開発しました。この機能は、残高が一定額を下回ると、ユーザーのクレジットカードまたはデビットカードから自動的に資金をチャージします。
- Azure EventHubからの決済後イベントを監視し、ユーザーが自動チャージ設定をセットアップした際に常にチャージを確認するために、シームレスな統合を確保しました。
- オブジェクト指向プログラミングの原則を適用して、自動チャージテーブルの変更に対する監査ログを作成するためにアスペクト指向プログラミングを使用しました。
- チームをAWS移行プロセスで支援し、IAMとRDSの設定をトラブルシューティングし、スムーズな移行を確保しました。
...
- システム全体を横断してリクエストを追跡するためのエンドツーエンドトレースIDソリューションを実装しました。
...
- 会計、元帳、銀行システムの専門知識を活かして、財務データの処理機能（インポート、検証、エクスポート）を維持し、新しい財務ヘッダーの提出と承認ワークフローを強化しました。
- ローカル開発からUATテスト、プロダクションリリースまでのフル開発ライフサイクルを管理し、Copilotを使用してルート原因分析、Pythonスクリプトの生成、約50の技術ガイドの作成を行い、これらはチームメンバーにとって重要な参考資料となりました。
- CopilotのようなAIツールを使用して革新的な銀行技術ソリューションを開発し、YourKitを使用してパフォーマンスプロファイリングを行い、システマティックなテストを実施し、プロジェクトアーキテクチャを分析し、チームと協力し、包括的なロギングとドキュメントを通じて改善しました。
- アスペクトJを使用して広範なログを収集し、デバッグ状態をテキストに変換し、AIチャットボットにコンテキストを提供するために、主要な金融プロジェクトにAIソフトウェアエージェントを実装する可能性を探りました。