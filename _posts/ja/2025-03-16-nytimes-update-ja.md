...
lang: ja
layout: post
title: ニューヨークタイムズニュース（CN）記事更新トリガー
translated: true
---

以下のボタンをクリックして、[NYTimes CN記事](./nytimes-en)を更新します。

<script async src="../assets/js/nytimes.js"></script>

<div class="nytimes" ></div>

このページでは、NYTimes CN記事の更新をトリガーできます。ボタンをクリックすると、最新の記事を取得し、翻訳し、このサイトのコンテンツを更新するワークフローが開始されます。更新をトリガーした後、変更が表示されるまで数分かかることがあります。
...
def call_mistral_api(prompt, model="mistral-small-2501"):
    ...
    try:
        ...
        print(f"送信されるプロンプト: {prompt[:1000]}...")  # プロンプトの最初の100文字を表示
        response = requests.post(url, headers=headers, json=data)
        response.raise_for_status()
        response_json = response.json()
        print(f"Mistral APIレスポンス: {response_json}")
        ...
        if e.response:
            ...
            print(f"レスポンスコンテンツ: {e.response.text}")
        return None
...
def fetch_html_content(url):
    ...
    try:
        ...
        response.raise_for_status()  # 4xxまたは5xxの応答に対してHTTPErrorを発生
        print(f"{url}からHTMLコンテンツを正常に取得しました")
        return response.text
    except requests.exceptions.RequestException as e:
        print(f"URL: {url}を取得できませんでした - {e}")
        return None
...
def translate_title(title):
    ...
    base_prompt = "次のタイトルを{target_language}に翻訳してください。追加の説明やノートは含めないでください。入力テキストを繰り返さないでください。\n"
    prompt = base_prompt.format(target_language="English") + f"{title}"
    print(f"タイトルを翻訳中: {title}")
...
def summarize_article(html):
    ...
    prompt = f"次の記事を英語で要約してください。主要なポイントに焦点を当て、導入文句のような「要約:」や「この記事は:」を避けてください。\n\n{article_text[:30000]}\n\n"  # 記事テキストを30000文字に制限
    print(f"タイトル: {title}の要約を作成中")
    summary = call_mistral_api(prompt)

    if summary:
        # 要約を「要約:」や類似のフレーズを削除してクリーンアップ
...
def update_markdown_file(filename, markdown_content):
    ...
    try:
        # ファイルの既存コンテンツを読み取ります
        print(f"{filename}の既存コンテンツを読み取ります")
        with open(filename, 'r', encoding='utf-8') as f:
            existing_content = f.read()

        # 初期メタデータの後にコンテンツの開始および終了位置を検索
        start_index = existing_content.find('---', 3) + 4  # 2番目の'---'を検索し、それを超えて移動
        end_index = len(existing_content)

        # 更新されたコンテンツを構築
        updated_content = existing_content[:start_index].strip() + '\n\n'  # メタデータを保持し、改行を追加
        updated_content += markdown_content.strip() + '\n'  # 新しいMarkdownリストを追加
        # updated_content += existing_content[end_index:].strip() # リストの後に存在するものを追加、存在する場合
...
def main():
    ...
    print(f'メインページに{len(links)}リンクが見つかりました。リンクを抽出中...')
    ...
    for i, link in enumerate(links):
        ...
        print(f'リンク{i + 1}の{len(links)}を処理中: {url}')
    ...
    if markdown_changed:
        print("新しいリンクでMarkdownファイルが更新されました。")
        sys.exit(0)
    else:
        print("Markdownファイルが更新されませんでした（変更なし）。")
        sys.exit(1)
    ...
    updateButton.addEventListener('click', () => {
        ...
        .then(response => {
            if (response.status === 204) {
                alert('更新が正常にトリガーされました！結果を確認するには数分待ちください。');
            } else {
        ...
        .catch(error => {
            ...
            console.error('更新のトリガー中にエラーが発生しました:', error);
        });
    });
} else {
