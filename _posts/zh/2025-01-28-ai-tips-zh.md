...
* 虽然 Cursor 中的 AI 模型有时会给出部分正确的文本，但我们可以接受它们，因为我们可以添加后续指令，使 AI 模型重新生成正确的部分。

* 如果不太可能有帮助，就不要向大型语言模型提供过多的上下文。例如，在生成对话对话行时，不要提供 100 个关于某个主题的点。大型语言模型已经包含大量数据。

* 在翻译或生成对话歌词等任务中提供充足的上下文时，避免使用链式思考功能，因为它可能会变慢，并导致冗长或无用的响应。

* 测试聊天机器人是否能遵循用户指令的一种方法是，让它用英语解释某事，然后继续用中文输入，观察聊天机器人是否保持输出为英语。

* 不要向 LLMs 提供上下文，而是基于大量文本或代码的数据集微调模型，然后使用微调后的模型。

* 每周轮流使用 AI 聊天机器人，以了解它们的差异。如果想了解更多，不要只使用一个 AI 工具或最好的工具。

* 打开 Grok、Gemini、ChatGPT、DeepSeek、Mistral、Perplexity 和 Claude 并使用它们，打开许多标签页，阅读它们的答案，然后提问。我们可以比较答案或根据我们的兴趣继续提问。
...
* 与 AI 聊天时，语言很重要，特别是涉及特定地区或领域时。使用相关本地知识可以帮助您找到更准确的信息——例如，如何获取身份证或注册车辆。
