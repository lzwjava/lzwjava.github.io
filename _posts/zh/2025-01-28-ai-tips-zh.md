...
* 使用 Deepseek 或 Mistral API 翻译标题时，提示如 `You are a professional translator. You are translating a markdown file for a Jekyll blog post from English to Chinese. {text}` 可能会导致错误的翻译。除了您提供的文本外，输出通常还会包含过多的翻译。

* 虽然有时 Cursor 中的 AI 模型会给出部分正确的文本，但我们可以接受它们，因为我们可以添加后续指令，使 AI 模型重新生成正确的部分。

* 如果不太可能有帮助，避免向大型语言模型提供过多的上下文。例如，在生成对话对话行时，避免提供有关主题的 100 个要点。大型语言模型已经包含大量数据。

* 在翻译或生成对话歌词等任务中提供充分的上下文时，避免使用链式思维功能，因为这可能会变慢并导致冗长或无用的响应。

* 测试聊天机器人是否能遵循用户指令的一种方法是要求它用英语解释某事，然后继续用中文输入，观察聊天机器人是否保持其输出为英语。

* 与其向 LLM 提供上下文，不如基于大量文本或代码的数据集微调模型，然后使用微调后的模型。

* 每周轮流使用 AI 聊天机器人，以了解它们的差异。如果想了解更多，不要只使用一种 AI 工具或最佳工具。
...
* 与其使用深度思考或链式思维（CoT）解决难题，有时可以简化问题，让标准模型解决它们。
