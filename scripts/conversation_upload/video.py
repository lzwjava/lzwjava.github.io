...
def get_mp3_duration(mp3_file):
    ...
    try:
        ...
        print(f"Warning: Could not get duration of {mp3_file}. Progress percentage won't be shown. Error: {e}")
        return None
...
def run_ffmpeg_with_progress(ffmpeg_cmd, mp3_file):
    ...
    cmd = ["ffmpeg", "-y"] + ffmpeg_cmd + ["-progress", "pipe:1", "-nostats", "-loglevel", "info"]
            ...
            if stream == process.stderr:
                ...
                if match:
                    ...
                    if duration:
                        ...
                        print(f"Progress: {time_ms:.1f}s / {duration:.1f}s ({percentage:.1f}%)")
                    else:
...
if font is None:
    ...
    print("Warning: Neither Arial nor Helvetica font found, using default font. Note: Default font may not scale well to 72pt.")
