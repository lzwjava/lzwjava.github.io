...
def explore_photos_database(photos_library_path):
    ...
    if not database_path.exists():
        ...
        if alternatives:
            ...
            print("Could not find Photos database. It might be in a different location or format.")
            return
    ...
    try:
        ...
        for table in tables:
            table_name = table[0]
            if "VIDEO" in table_name.upper() or "MEDIA" in table_name.upper() or "ASSET" in table_name.upper():
                video_related_tables.append(table_name)
        
        print(f"\nPotential video-related tables: {video_related_tables}")
        
        # Examine video tables
        for table in video_related_tables[:5]:  # Limit to first 5 to avoid overwhelming output
            try:
                ...
                path_cols = [col[1] for col in columns if 'PATH' in col[1].upper() or 'FILE' in col[1].upper()]
                ...
                if size_cols and count > 0:
                    ...
                    try:
                        ...
                        query = f"SELECT * FROM {table} ORDER BY {size_col} DESC LIMIT 5;"
                        ...
                        for type_col in type_cols:
                            try:
                                ...
                                if rows:
                                    ...
                                    df = pd.DataFrame(rows, columns=col_names)
                                    print(df[[size_col, type_col] + [c for c in col_names if 'PATH' in c.upper() or 'NAME' in c.upper()][:3]])
                            except sqlite3.Error as e:
