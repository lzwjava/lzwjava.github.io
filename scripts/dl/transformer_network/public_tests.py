...
def Decoder_test(target, create_look_ahead_mask, create_padding_mask):
    ...
    assert np.allclose(outd[1, 1],
                       [-0.30814743, -0.6213016, -0.77767026, 1.7071193]), "Wrong values in outd when training=True"

    outd, att_weights = decoderk(x, encoderq_output, True, look_ahead_mask, create_padding_mask(x))
    assert np.allclose(outd[1, 1], [-0.0250004, 0.50791883, -1.5877104,
                                    1.1047921]), "Wrong values in outd when training=True and use padding mask"
