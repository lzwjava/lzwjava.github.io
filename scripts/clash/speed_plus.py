...
def get_top_proxies(num_results=10):
    logging.info("Starting Clash proxy speed test against top websites (concurrently)...")
    ...
    with ThreadPoolExecutor(max_workers=CONCURRENT_CONNECTIONS) as executor:
        future_to_proxy = {executor.submit(get_average_latency, name): name for name in proxy_names_to_test}
        ...
        for future in as_completed(future_to_proxy):
            ...
            try:
                ...
                if average_latency is not None:
                    ...
                    logging.info(f"Proxy: {proxy_name} - Test Failed or Timed Out for all websites.")
            except Exception as exc:
                logging.error(f"Proxy {proxy_name} generated an unexpected exception during testing: {exc}")

    sorted_proxies = sorted([item for item in proxy_average_latencies.items() if item[1] is not None],
    ...
    if not sorted_proxies:
        ...
        logging.info(f"--- Top {num_results} Fastest Individual Proxies (Average Latency) ---")
